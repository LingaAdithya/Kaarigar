...
import { addProduct, NewProduct } from '@/services/artisan-service';
...
export default function AddProductPage() {
  ...
  const handleFinalSubmit = async (useEnhancedImage = true) => {
    ...
    try {
      await addProduct(formData as NewProduct, imageToSubmit);
      toast({
    ...
    switch (pageState) {
      ...
      case PageState.SUBMITTING:
        return (
            <div className="text-center p-8 flex flex-col items-center justify-center h-96">
                <Loader2 className="h-12 w-12 animate-spin text-primary mb-4" />
                <h2 className="font-headline text-2xl mb-2">Listing your Masterpiece...</h2>
                <p className="text-muted-foreground">Just a few more seconds.</p>
            </div>
        );
      case PageState.FORM:
      default:
            ...
            <CardContent>
              <form onSubmit={handleCreateListing}>
                <div className="grid gap-6">
                  ...
                  <div className="space-y-2">
                    <Label>2. Describe with your voice</Label>
                    <Button type="button" variant="outline" className="w-full" onClick={toggleRecording} disabled={isGeneratingDetails || !originalImage}>
                        ...
                        {!isRecording && !isGeneratingDetails && <Mic className="mr-2 h-4 w-4" />}
                        {isGeneratingDetails && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                        {isRecording ? 'Stop Recording' : (isGeneratingDetails ? 'Generating Details...' : 'Start Recording')}
                    </Button>
                    {!originalImage && <p className="text-xs text-muted-foreground text-center">Please upload a photo before recording.</p>}
                  </div>
       ...
       {pageState === PageState.FORM && (
        <>
            ...
            <p className="text-muted-foreground text-center mb-8">
                Fill in the details below or use your voice to describe your creation.
            </p>
        </>
       )}
